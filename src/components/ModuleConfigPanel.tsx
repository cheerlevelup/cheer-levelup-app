...
/**
 * ModuleConfigPanel — konfiguracja wellness dla grupy lub zawodniczki.
...
const WELLNESS_PRESETS = [
  { label: 'Minimalne',    pre: ['sleep_hours', 'sleep_quality', 'readiness', 'energy', 'stress'] },
  { label: 'Standardowe', pre: ['sleep_hours', 'sleep_quality', 'readiness', 'energy', 'stress', 'muscle_soreness', 'hydration', 'recovery_score'] },
  { label: 'Pełne',       pre: WELLNESS_DAILY.map(p => p.id) },
  { label: 'Wyczyść',     pre: [] },
]
...
export default function ModuleConfigPanel({ groupId, athleteId, module, groupConfig, onClose }: Props) {
  ...
  function toggle(id: string) {
    setSaved(false)
    setPre(prev => prev.includes(id) ? prev.filter(x => x !== id) : [...prev, id])
  }

  function applyPreset(p: { label: string; pre: string[] }) {
    setPre(p.pre); setSaved(false)
  }
  ...
  async function handleSave() {
    ...
    setError('')
    const payload: any = {
      module, enabled, pre_params: pre, post_params: [],
      ...
      <div style={{ width: '100%', maxWidth: 620, background: C.white, borderRadius: 18, border: `1.5px solid ${C.grayLight}`, maxHeight: '92vh', display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
        ...
        <div style={{ padding: '0.875rem 1.25rem', borderBottom: `1.5px solid ${C.grayLight}`, flexShrink: 0, display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 12 }}>
          <div>
            <div style={{ fontWeight: 700, fontSize: '0.9rem', color: C.navy }}>
              {enabled ? '✅ Monitoring wellness włączone' : '🚫 Monitoring wellness wyłączone'}
            </div>
          ...
          <>
            ...
            <div style={{ padding: '0.65rem 1.25rem', borderBottom: `1.5px solid ${C.grayLight}`, flexShrink: 0 }}>
              ...
              <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap' }}>
                ...
                {isAthlete && groupConfig && (
                  <button onClick={() => { setPre(groupConfig.pre); setSaved(false) }}
            ...
            <div style={{ padding: '0.5rem 1.25rem', borderBottom: `1.5px solid ${C.grayLight}`, flexShrink: 0 }}>
              <span style={{ fontFamily: mono, fontSize: '0.65rem', color: C.gray }}>
                {pre.length} pól dziennego wellness
              </span>
            </div>
            ...
            <div style={{ overflowY: 'auto', flex: 1, padding: '0.65rem 1rem' }}>
              <div style={{ display: 'flex', flexDirection: 'column', gap: 5 }}>
                {WELLNESS_DAILY.map(param => {
                  const on = pre.includes(param.id)
                  const inheritedOn = groupConfig?.pre.includes(param.id)
                  return (
                    <button key={param.id} onClick={() => toggle(param.id)} style={{ display: 'grid', gridTemplateColumns: '32px 1fr auto', alignItems: 'center', gap: 10, padding: '0.55rem 0.75rem', borderRadius: 10, border: `1.5px solid ${on ? C.gold : C.grayLight}`, background: on ? `${C.navy}08` : C.offWhite, cursor: 'pointer', textAlign: 'left', transition: 'all 0.15s' }}>
        ...
        <div style={{ padding: '0.75rem 1.25rem', borderTop: `1.5px solid ${C.grayLight}`, display: 'flex', gap: 8, alignItems: 'center', flexShrink: 0 }}>
          <div style={{ fontFamily: mono, fontSize: '0.6rem', color: C.gray, flex: 1 }}>
            {enabled ? `${pre.length} pól dziennego wellness` : 'Arkusz wyłączony'}
          </div>
