...
export default function CoachPlansClient({ plans }: Props) {
        ...
        <header style={{ background: C.navy, padding: '1rem 1.25rem 1.35rem', position: 'sticky', top: 0, zIndex: 10 }}>
          <div style={{ maxWidth: 720, margin: '0 auto' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              ...
              <button onClick={() => router.push('/coach/plans/new')} style={{ border: 'none', background: C.gold, color: C.navy, borderRadius: 10, padding: '0.65rem 0.85rem', fontWeight: 800, fontSize: '0.82rem' }}>
                Nowy plan
              </button>
            </div>
            <h1 style={{ color: C.white, fontSize: '1.45rem', fontWeight: 800, marginTop: '1rem' }}>Plany treningowe</h1>
            <p style={{ color: C.gray, fontSize: '0.84rem', marginTop: 4 }}>Biblioteka planow i edycja tygodni.</p>
          </div>
        </header>
          ...
          {plans.length === 0 ? (
            <Card>
              <div style={{ textAlign: 'center', padding: '2rem 1rem' }}>
                <div style={{ fontWeight: 800, fontSize: '1rem', marginBottom: 6 }}>Brak planow treningowych</div>
                <div style={{ color: C.gray, fontSize: '0.85rem', marginBottom: '1rem' }}>Utworz pierwszy plan dla swoich zawodniczek.</div>
                <button onClick={() => router.push('/coach/plans/new')} style={{ padding: '0.85rem 1rem', background: C.navy, color: C.gold, border: 'none', borderRadius: 10, fontWeight: 800 }}>
                  Utworz pierwszy plan
                </button>
              </div>
            </Card>
          ) : (
            <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
              {plans.map(plan => (
                <Card key={plan.id}>
                  <button onClick={() => router.push(`/coach/plans/${plan.id}`)} style={{ width: '100%', background: 'none', border: 'none', padding: '1rem', display: 'flex', alignItems: 'center', justifyContent: 'space-between', textAlign: 'left' }}>
